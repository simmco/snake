...
const getIsOppositeDirection = (currentDirection, newDirection) => {
  let statement = false;
  if (currentDirection === "UP" && newDirection === "BOTTOM") statement = true;
  if (currentDirection === "BOTTOM" && newDirection === "UP") statement = true;
  if (currentDirection === "LEFT" && newDirection === "RIGHT") statement = true;
  if (currentDirection === "RIGHT" && newDirection === "LEFT") statement = true;
  return statement;
};
...
const reducer = (state, action) => {
  switch (action.type) {
    case "SNAKE_CHANGE_DIRECTION":
      const currentDirection = state.playground.direction;
      const isOppositeDirection = getIsOppositeDirection(
      ...
      return {
        ...
        playground: {
          ...state.playground,
          direction: isOppositeDirection ? currentDirection : action.direction
        }
      };
    case "SNAKE_MOVE":
    ...
    case "GAME_OVER":
      localStorage.setItem("snake-highscore", state.snake.coordinates.length);
      return {
        ...state,
        playground: {
          ...state.playground,
...
const App = () => {
  ...
  return (
    <div className="app">
      ...
      />
      <div className="score">
        <span>SCORE: {state.snake.coordinates.length}</span>
        <span>HIGH SCORE: {localStorage.getItem("snake-highscore")} </span>
      </div>
    </div>
  );
};
...
const Cell = ({ isGameOver, snake, snack, x, y }) => (
  <div className={getCellCs(isGameOver, snake, snack, x, y)}>
    {isPosition(x, y, snack.coordinate.x, snack.coordinate.y) && (
      <React.Fragment>
        <div className="grid-cell-snack-block-row">
          <div className="grid-cell-snack-block" />
          <div className="grid-cell-snack-block grid-cell-snack-block-colored" />
          <div className="grid-cell-snack-block" />
        </div>
        <div className="grid-cell-snack-block-row">
          <div className="grid-cell-snack-block grid-cell-snack-block-colored" />
